...
const Expenditure = () => {
  ...
  useEffect(() => {
    const fetchAssets = async () => {
      ...
      //geting token from Authentication
      if (!token) {
      ...
      try {
        const res = await fetch(
          `https://servermms.onrender.com/api/expenditures?sortBy=expenditureDate&sortOrder=desc&limit=${limit}&skip=${
            (page - 1) * limit
          }`,
          {
            headers: {
        ...
        toast.success("Successfully loaded data ", total);
      }
    };
